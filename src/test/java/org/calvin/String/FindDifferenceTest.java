/*
 * Copyright Calvin Lee Since 2016.
 * All Rights Reserved.
 */

package org.calvin.String;

import org.junit.Before;
import org.junit.Test;

import static org.junit.Assert.assertEquals;

public class FindDifferenceTest {
    private FindDifference fixture;

    @Before
    public void setUp() {
        fixture = new FindDifference();
    }

    @Test
    public void shouldFindTheDifferenceSimpleCase() {
        char actual = fixture.findTheDifference("ae", "aea");
        assertEquals('a', actual);
    }

    @Test
    public void shouldNotFindTheDifference() {
        char actual = fixture.findTheDifference("ae", "ae");
        assertEquals(' ', actual);
    }

    @Test
    public void shouldFindTheDifferenceLongString() {
        char actual = fixture.findTheDifference
                ("",
                "");
        assertEquals('t', actual);
    }

    @Test
    public void shouldFindTheDifferenceXOR() {
        char actual = fixture.findTheDifferenceXOR("ae", "aea");
        assertEquals('a', actual);
    }

    @Test
    public void shouldNotFindTheDifferenceXOR() {
        char actual = fixture.findTheDifferenceXOR("ae", "ae");
        assertEquals(' ', actual);
    }

    @Test
    public void shouldFindTheDifferenceLongStringXOR() {
        char actual = fixture.findTheDifferenceXOR
                ("",
                        "");
        assertEquals('t', actual);
    }

    @Test
    public void shouldFindTheDifferenceXORMyWay() {
        char actual = fixture.findTheDifferenceBucket("ae", "aea");
        assertEquals('a', actual);
    }

    @Test
    public void shouldNotFindTheDifferenceXORMyWay() {
        char actual = fixture.findTheDifferenceBucket("ae", "ae");
        assertEquals(' ', actual);
    }

    @Test
    public void shouldFindTheDifferenceLongStringXORMyWay() {
        char actual = fixture.findTheDifferenceBucket
                ("",
                        "");
        assertEquals('t', actual);
    }
}
